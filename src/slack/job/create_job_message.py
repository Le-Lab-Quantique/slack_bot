...
def create_confirm_or_reject_message(
    posted_job: CreatedJobResult, job_id: int
) -> Sequence[Union[dict, Block]]:
    ...
    image_url = posted_job.partner.partner_logo.node.media_item_url
    alt_text = posted_job.partner.partner_logo.node.alt_text or "partner logo"
    image = ImageElement(image_url=image_url, alt_text=alt_text).to_dict()
